...
void printMST(int visited_nodes[], int size, int version, int **tab) {
	if (version == 1) cout << "wierzcholki : ";
	else cout << "list   : ";
	for (int i = 0; i < size; i++)
		cout << visited_nodes[i]+1 << " ";
	cout << endl << "krawedzi : ";
	int suma = 0;
	for (int i = 1; i < size; i++)
	{
		cout <<"("<< visited_nodes[i - 1]+1<<","<<visited_nodes[i]+1 << ") ";
	}
	cout << endl<<"wagi : ";
	for (int i = 1; i < size; i++)
	{
		cout << tab[visited_nodes[i - 1]][visited_nodes[i]] << " ";
		suma += tab[visited_nodes[i - 1]][visited_nodes[i]];
	}
	cout << endl << "suma wag : " << suma << endl;
}
...
void table_reader(int **tab, int size) {
	ifstream in("mst.txt");
	in >> size;
	for (int i = 0; i < size; i++)
	{
...
void matrix_MST(int **tab, int size) {
	...
	for (int count = 0; count < size; count++)
	{
		...
		printMST(visited_nodes, size, 1, tab);
	//	cout<<min_length<<endl;
	delete accessible_lines_values;
	delete visited_nodes;
	delete is_visited;
